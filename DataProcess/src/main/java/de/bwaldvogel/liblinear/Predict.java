...
public class Predict {
     ...
     */
    static double doPredict(BufferedReader reader, Writer writer, Model model, String resultPath) throws IOException {
        int correct = 0;
        int total = 0;
        double result = 0.0;
        double error = 0;
        ...
        while ((line = reader.readLine()) != null) {
			...
			if(target_label == 1.0 && predict_label == 1.0) vv++;
			if(target_label == -1.0 && predict_label == -1.0) yy++;
			if(target_label == -1.0 && predict_label == 1.0) yv++;
			if(target_label == 1.0 && predict_label == -1.0) vy++;
			accP =  vv/(vv+yv);
			recallP = vv/(vv+vy);
    ...
    public static double main(String[] argv, String resultPath) throws IOException {
        ...
        try {
            ...
            result = doPredict(reader, writer, model, resultPath);
        }
        finally {
            closeQuietly(reader);
            closeQuietly(writer);
